...
import { Star, Mic, ImageOff, Calendar, Heart, Shield, Award, Eye, Languages } from 'lucide-react';
...
const PlatformIcon: React.FC<{ platform: string }> = ({ platform }) => {
  ...
  const handleError = () => {
    ...
    if (imgSrc.includes('clearbit')) {
      ...
      // If Google fails (or logic falls through), show text fallback
      setError(true);
    }
  };
...
const MediaCard: React.FC<MediaCardProps> = ({ item }) => {
  ...
  return (
    <div 
        ...
        className="group relative bg-white/[0.02] backdrop-blur-2xl border border-white/[0.05] rounded-2xl overflow-hidden cursor-pointer hover:bg-white/[0.08] hover:border-white/20 hover:shadow-[0_8px_32px_0_rgba(255,255,255,0.1)] transition-all duration-500 transform hover:-translate-y-2 h-full flex flex-col shadow-glass shadow-glass-inset ring-1 ring-white/5"
    >
    {/* Poster Image */}
    <div className="aspect-[2/3] relative overflow-hidden bg-white/5">
        {!hasError ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center p-4 text-slate-500 bg-white/5">
        ...
        {/* Glass Hover Overlay with Updated Button */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 z-10 items-center">
        ...
        {/* Watchlist Toggle - Top Left - More Glassy */}
        <button 
            ...
            className={`
                ...
                ${inWatchlist 
                    ...
                    : 'bg-black/20 border-white/10 text-slate-300 hover:text-white hover:bg-white/20'}
            `}
        ...
        <div className="absolute top-2 right-2 flex flex-col gap-1.5 items-end z-20">
            ...
            {item.maturityRating && (
                <span className="bg-black/40 backdrop-blur-md border border-white/10 text-white text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1 shadow-sm">
                     {item.maturityRating}
                </span>
            )}
            ...
            <div className="flex gap-1">
                ...
                {originalLang && originalLang !== 'Unknown' && (
                    <span className="bg-slate-700/60 backdrop-blur-md text-white text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1 shadow-lg border border-slate-500/30">
                        {originalLang}
                    </span>
                )}
                ...
                {/* Dub Badge */}
                {isDubbed && (
                    <span className="bg-blue-600/60 backdrop-blur-md text-white text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1 shadow-lg border border-blue-400/30">
                        <Mic size={10} /> DUB
                    </span>
                )}
            </div>
            ...
            {/* Tech Specs / Quality */}
            {techSpecs.length > 0 && (
                <span className="bg-purple-600/40 backdrop-blur-md text-white text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1 shadow-lg border border-purple-400/20">
    ...
    {/* Content Info - Lighter Glass Background */}
    <div className="p-4 flex flex-col flex-grow relative bg-transparent">
        {/* Subtle top shine */}
