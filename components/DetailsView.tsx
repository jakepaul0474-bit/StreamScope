...
import { ArrowLeft, Star, Calendar, Globe, MonitorPlay, ShieldAlert, Mic, Info, ImageOff, Clock, ChevronDown, ChevronUp, PlayCircle, Heart, Check } from 'lucide-react';
...
const DetailsView: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen text-white">
        <div className="flex flex-col items-center gap-4 p-8 bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10">
  ...
  const chartData = item.ratingsBreakdown ? [
    { name: 'Story', score: item.ratingsBreakdown.story },
    { name: 'Acting', score: item.ratingsBreakdown.acting },
    { name: 'Visuals', score: item.ratingsBreakdown.visuals },
    { name: 'Sound', score: item.ratingsBreakdown.sound },
  ] : [
    { name: 'Story', score: item.imdbRating },
    { name: 'Acting', score: item.imdbRating },
    { name: 'Visuals', score: item.imdbRating },
    { name: 'Sound', score: item.imdbRating },
  ];
  ...
  return (
    <div className="min-h-screen pb-20 pl-0 md:pl-20 lg:pl-64 transition-all duration-300 overflow-x-hidden">
      ...
      <div className="relative h-[45vh] w-full bg-slate-900 overflow-hidden">
        ...
        <button 
          onClick={() => navigate(-1)}
          className="absolute top-6 left-6 bg-white/10 backdrop-blur-md border border-white/10 p-3 rounded-full hover:bg-white/20 transition-all z-20 group shadow-lg"
        >
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-40 relative z-10">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="w-full md:w-72 flex-shrink-0">
            <div className="w-full aspect-[2/3] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 bg-slate-800/50 backdrop-blur-sm overflow-hidden relative z-20">
                {!posterError ? (
                    <img 
                      src={posterSrc} 
                      alt={item.title} 
            ...
            <div className="mt-6 bg-white/5 backdrop-blur-xl p-5 rounded-2xl border border-white/10 shadow-lg">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                <MonitorPlay size={14} /> Available On
              </h3>
              <div className="flex flex-wrap gap-2">
                {item.platforms && item.platforms.length > 0 ? item.platforms.map(platform => (
                  <span key={platform} className="px-3 py-1.5 bg-primary/10 text-primary border border-primary/20 rounded-lg text-sm font-medium hover:bg-primary/20 transition-colors">
                    {platform}
                  </span>
                )) : <span className="text-slate-500 text-sm italic">Not currently available for streaming</span>}
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 pt-4 md:pt-12">
            ...
            <div className="flex flex-wrap gap-4 mb-8 items-center justify-between">
                ...
                <button 
                    ...
                    className={`
                        flex items-center gap-2 px-6 py-2.5 rounded-full font-bold transition-all duration-300 border
                        ${inWatchlist 
             ...
             {item.nextEpisode && (
                <div className="mb-8 bg-gradient-to-br from-blue-900/40 to-purple-900/40 backdrop-blur-md border border-white/10 p-1 rounded-2xl shadow-xl">
                   <div className="bg-white/5 rounded-xl p-5 flex flex-col sm:flex-row gap-5 items-center justify-between">
            ...
            {item.contentAdvisory && (
                <div className="mb-8 bg-red-900/20 backdrop-blur-md border border-red-500/20 p-4 rounded-2xl flex gap-4 items-start shadow-lg">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="flex flex-col gap-4">
                            {item.seasons.map((season) => (
                                <div key={season.seasonNumber} className="bg-white/5 backdrop-blur-md border border-white/5 rounded-2xl overflow-hidden transition-all duration-300 hover:bg-white/10 hover:border-white/10">
                                    <button 
              ...
              {/* D3/Recharts Visualization - Glass Panel */}
              <div className="bg-white/5 backdrop-blur-xl p-5 rounded-3xl border border-white/10 h-fit sticky top-24 shadow-2xl">
                 <h3 className="text-sm font-bold text-slate-400 mb-6 uppercase tracking-wider text-center">Rating Breakdown</h3>
                      ...
                      <BarChart data={chartData} layout="vertical" margin={{ left: 40, right: 20 }}>
                        ...
                        <Bar dataKey="score" radius={[0, 8, 8, 0]} barSize={24}>
                          {chartData.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={['#3b82f6', '#8b5cf6', '#f43f5e', '#10b981'][index % 4]} />
                          ))}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                 </div>
                 <div className="flex justify-between px-4 mt-2 text-[10px] text-slate-500 font-mono">
